// cs/apps/common/file-viewer/file_viewer.gpt.cc
...
static std::string MakeBreadcrumbs(
  ...
  for (auto it = p.begin(); it != p.end(); ++it) {
    ...
    const std::string link =
        a({}, BuildHref(base_path, UrlEncode(accum)),
...
static ResultOr<std::string> MakePage(
    ...
    const Request& request, const std::string& nav_label,
  ...
  std::string viewer_domain;
  {
    NavItemSpec item;
    SET_OR_RET(viewer_domain,
               GetDomainForService(service_name, request));
    item.href = "https://" + viewer_domain + "/";
  ...
  if (current_host == viewer_domain) {
    opts.active_path = "https://" + viewer_domain + "/";
  } else {
    opts.active_path = "";
  }
  opts.content_html =
...
static ResultOr<std::string> RenderHtmlForPath(
    const std::string& relative_path,
    const std::filesystem::path& base_dir,
    const ::cs::apps::common::file_viewer::
        FileViewerOptions& options) {
  if (PathContainsDotGit(relative_path)) {
    return TRACE(PermissionDenied(
        "Access denied: .git is not "
        "allowed in the " +
        options.access_denied_suffix + "."));
  }
  ...
  if (std::filesystem::is_directory(target_canon)) {
    ...
    if (!rel_p.empty() && rel_p != ".") {
      ...
      parent_str =
          BuildHref(normalized_base, UrlEncode(parent_str));
  ...
  if (std::filesystem::is_regular_file(target_canon)) {
    ...
    if (options.pretty_print_json &&
        target_canon.extension() == ".json") {
      std::string content(
          (std::istreambuf_iterator<char>(ifs)),
          std::istreambuf_iterator<char>());
      auto obj_or = ParseObject(content);
      if (!obj_or.ok()) {
        html << MakeBreadcrumbs(
            relative_path.empty()
                ? target_canon.filename().string()
                : relative_path,
            normalized_base, options.breadcrumb_root);
        html << hr();
        html << Alert("danger", obj_or.message());
        return html.str();
      }
      std::string formatted =
          SerializeObject(obj_or.value(), 4);
      ...
      html << hr();
      if (!e3) {
        std::string meta_html = span(
            {{"class", "meta"}},
            "Size: " + std::to_string(fsize) + " bytes. ");
        if (options.show_github_link &&
            !options.github_base_url.empty()) {
          std::string github_path = relative_path;
          if (github_path.empty() || github_path == ".") {
            github_path = "";
          }
          std::string github_url = options.github_base_url +
    ...
    for (size_t i = 0; i < nlines; ++i) {
      ln_ss << std::setw((int)width) << std::setfill(' ')
            << (i + 1) << '\n';
      code_ss << ::cs::apps::common::file_viewer::
                     RenderCodeLineWithQuotedIncludeLinks(
                         lines[i],
                         options.link_quoted_includes,
                         relative_path, normalized_base,
                         options.breadcrumb_root.empty()
                             ? ""
                             : (options.breadcrumb_root +
                                "/"))
              << '\n';
...
Response GetFileViewerPage(
  ...
  auto html_or = RenderHtmlForPath(
      rel_path, options.base_dir, options);
  if (!html_or.ok()) {
    return ErrorResponse(html_or);
  }

  auto page_or =
      MakePage(options.title, html_or.value(), request,
               options.nav_label, options.service_name);
  if (!page_or.ok()) {
