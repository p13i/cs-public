// cs/q/trees/iterate/post_order_test.gpt.cc
...
std::vector<T> Collect(Node<T>* root) {
  std::vector<T> out;
  PostOrderTreeIterator<T> it(root);
  while (it.HasNext()) {
...
//        (empty)
// expected: {}
TEST(PostOrderTreeIteratorGPT, EmptyTree) {
...
//      1
// expected: 1
TEST(PostOrderTreeIteratorGPT, SingleNode) {
...
//   1
// expected: 1 2 3
TEST(PostOrderTreeIteratorGPT, LeftSkewed) {
...
//       3
// expected: 3 2 1
TEST(PostOrderTreeIteratorGPT, RightSkewed) {
  Node<int>* root =
...
TEST(PostOrderTreeIteratorGPT, PerfectBST) {
  Node<int>* root =
      NodeOf(4, NodeOf(2, NodeOf(1), NodeOf(3)),
             NodeOf(6, NodeOf(5), NodeOf(7)));
  EXPECT_THAT(Collect(root),
              ::testing::ElementsAre(1, 3, 2, 5, 7, 6, 4));
  DeleteTree(root);
}
...
//            /
//           18
// expected: 6 5 18 20 15 10
TEST(PostOrderTreeIteratorGPT, UnbalancedMixed) {
  Node<int>* root = NodeOf(
      10, NodeOf(5, nullptr, NodeOf(6)),
      NodeOf(15, nullptr, NodeOf(20, NodeOf(18), nullptr)));
  EXPECT_THAT(Collect(root),
              ::testing::ElementsAre(6, 5, 18, 20, 15, 10));
  DeleteTree(root);
}
...
TEST(PostOrderTreeIteratorGPT, HasNextDrains) {
  Node<int>* root =
      NodeOf(4, NodeOf(2, NodeOf(1), NodeOf(3)), NodeOf(5));
  PostOrderTreeIterator<int> it(root);
