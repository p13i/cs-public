// cs/net/proto/db/in_memory_client.gpt.cc
...
struct InMemoryDatabaseClient::Impl {
  std::map<std::string, std::map<std::string, std::string>>
      collections;
};

InMemoryDatabaseClient::InMemoryDatabaseClient()
    : impl_(std::make_unique<Impl>()) {}

InMemoryDatabaseClient::~InMemoryDatabaseClient() = default;

std::string InMemoryDatabaseClient::GetBaseUrl() const {
  return "in-memory://";
}

ResultOr<QueryResponse> InMemoryDatabaseClient::Query(
    const QueryRequest& request) const {
  auto it = impl_->collections.find(request.collection);
  ...
  for (const auto& [uuid, payload_json] :
    ...
    SET_OR_RET(auto json_obj, ParseObject(payload_json));
    bool matches = true;
