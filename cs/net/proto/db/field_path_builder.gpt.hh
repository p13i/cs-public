...
// Only member pointers or chain results
// (FieldProxy/FieldPathBuilder) are accepted. Paths always
// come from GetFieldPath; raw strings or other types are
// rejected to avoid silent errors in the query language.
template <typename T>
std::string GetFieldPathOrConvert(
    const FieldProxy<T>& arg) {
  return std::string(arg.get_path());
}
template <typename T>
std::string GetFieldPathOrConvert(
    const FieldPathBuilder<T>& arg) {
  return std::string(arg.get_path());
}
template <typename T>
std::enable_if_t<std::is_member_pointer_v<std::decay_t<T>>,
                 std::string>
GetFieldPathOrConvert(T&& arg) {
